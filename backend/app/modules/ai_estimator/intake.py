...
class IntakeService:
    ...
    async def answer(
        ...
        if recorded:
            transcript.append(
                {"role": "user", "text": self._format_answers(pt, spec.answers), "ts": _now_iso()}
            )

        await self.intake_repo.update_fields(
            intake.id, params=params, param_status=status, transcript=transcript
        )
        intake = await self._reload_intake(run.id)
    ...
    async def confirm_parameters(
        ...
        await self.intake_repo.update_fields(
            intake.id, params=params, param_status=status, phase="compose_groups"
        )
        intake = await self._reload_intake(run.id)
    ...
    async def _run_extract(
        ...
        detected, confidence, seeded, summary, degraded = await self._extract(
            run, intake, manual_type=manual_type
        )

        status = {k: _KNOWN for k in seeded}
        await self.intake_repo.update_fields(
            intake.id,
    ...
    async def _extract_ai(
        ...
        prompt = build_intake_extract_prompt(
            request_text=text, type_registry_digest=self._type_registry_digest()
        )
        try:
    ...
    async def _phrase_questions_ai(
        ...
        digest = "\n".join(
            f"- {p.key} | kind={p.kind} | unit={p.unit or ''} | choices={','.join(p.choices or ())}"
            for p in pending
        )
        ...
        *,
        ...
        for offset, res in enumerate(results):
            ...
            group = await self._persist_group(
                run, pkg, stage, res["desc"], res["score"], qty, sort_base + offset
            )
        ...
        await self.intake_repo.update_fields(
            intake.id, packages=list(merged.values()), phase="group_board"
        )
    ...
    def _seed_from_text(text: str) -> dict[str, Any]:
        ...
        Reuses ``parse_text_scope`` to read a leading "<number> <unit>" and
        maps an area onto ``floor_area_m2`` so "ремонт дома 120м2" pre-fills the
        area question. Only numbers the user actually wrote are seeded.
        """
    ...
    def _require_type(self, intake: AiEstimatorIntake, override: str | None = None) -> ProjectType:
        ...
        if pt is None:
            raise HTTPException(
                status_code=409, detail="No project type selected yet. Pick a type to continue."
            )
        return pt
    ...
    async def _reseed_type(self, run: AiEstimatorRun, intake: AiEstimatorIntake, type_key: str) -> None:
        ...
        await self.intake_repo.update_fields(
            ...
            params=seeded,
            param_status={k: _KNOWN for k in seeded},
            round_idx=0,
            questions=[],
            phase="extract",
        )
